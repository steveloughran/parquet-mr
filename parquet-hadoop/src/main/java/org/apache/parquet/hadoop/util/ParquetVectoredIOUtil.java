...
import static org.apache.hadoop.fs.shim.api.ShimFactory.shimFSDataInputStream;
...
public class ParquetVectoredIOUtil {
  ...
  public static void readVectoredAndPopulate(final List<ParquetFileRange> ranges,
    ...
    List<VectorFileRange> fileRanges = ranges.stream()
      .map(range -> VectorFileRange.createFileRange(range.getOffset(), range.getLength(), range))
      .collect(Collectors.toList());
    shimFSDataInputStream(stream).readVectoredRanges(fileRanges, allocate);
